describe('Landing Page', () => {
  ...
  it('Should show a drawer on hamburger click', () => {
    ...
    cy.get('.MuiDrawer-paper').within(() => {
      ...
      cy.get('.MuiListItemButton-root').eq(4).should('have.text', 'Saved Articles')
      cy.get('.MuiListItemButton-root').eq(5).should('have.text', 'Add Pet')
      cy.get('.MuiListItemButton-root').eq(6).should('have.text', 'Manage Account')
      cy.get('.MuiListItemButton-root').eq(7).should('have.text', 'Manage Pets')
    })
  })
})